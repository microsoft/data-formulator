    ...
    <Tooltip title="save session locally">
      <Button
        ...
        onClick={() => {
          ...
          download(
            fullStateJson,
            `data-formulator.${new Date().toISOString()}.dfstate`,
            "text/plain"
          );
        }}
...
export const toolName = "Omni Dashboard";
    ...
    <>
      ...
      <Dialog onClose={() => setOpen(false)} open={open}>
        ...
        <DialogContent>
          <DialogContentText>
            <Typography>
              All unexported content (charts, derived data, concepts) will be
              lost upon reset.
            </Typography>
          </DialogContentText>
        </DialogContent>
    ...
    <>
      ...
      >
        ...
        >
          <Box
            component="span"
            sx={{ py: 0, my: 0, fontSize: "10px", mr: "auto" }}
          >
            timeout={config.formulateTimeoutSeconds}s
          </Box>
          <Box
            component="span"
            sx={{ py: 0, my: 0, fontSize: "10px", mr: "auto" }}
          >
            max_repair={config.maxRepairAttempts}
          </Box>
        </Box>
      </Button>
      <Dialog onClose={() => setOpen(false)} open={open}>
        <DialogTitle>Omni Dashboard Configuration</DialogTitle>
        <DialogContent>
          <DialogContentText>
            <Box
              sx={{
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                <Box sx={{ flex: 1 }}>
                  ...
                  >
                    Maximum time allowed for the formulation process before
                    timing out.
                  </Typography>
                  <Typography
                    variant="caption"
                    ...
                    sx={{ mt: 1, display: "block" }}
                  >
                    Smaller values (&lt;30s) make the model fails fast thus
                    providing a smoother UI experience. Increase this value for
                    slow models.
                  </Typography>
                </Box>
              </Box>
              <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                <Box sx={{ flex: 1 }}>
                  ...
                  <Typography
                    ...
                    sx={{ mt: 1, display: "block" }}
                  >
                    Maximum number of times the LLM will attempt to repair code
                    if generated code fails to execute (recommended = 1).
                  </Typography>
                  <Typography
                    variant="caption"
                    color="text.secondary"
                    sx={{ mt: 1, display: "block" }}
                  >
                    Higher values might slightly increase the chance of success
                    but can crash the backend. Repair time is as part of the
                    formulate timeout.
                  </Typography>
                </Box>
              </Box>
...
export const AppFC: FC<AppFCProps> = function AppFC(appProps) {
  const visViewMode = useSelector(
    (state: DataFormulatorState) => state.visViewMode
  );
  const config = useSelector((state: DataFormulatorState) => state.config);
    ...
    <Box sx={{ display: "flex" }} key="switchers">
      <ToggleButtonGroup
        ...
        sx={{
          ...
          padding: "4px 0px",
          marginTop: "2px",
          "& .MuiToggleButton-root": { padding: "0px 6px" },
        }}
      >
    ...
    <AppBar className="app-bar" position="static" key="app-bar-main">
      <Toolbar variant="dense">
        ...
        >
          ...
          >
            Ravian AI {process.env.NODE_ENV == "development" ? "" : ""}
          </Typography>
        </Button>
        <Box
          sx={{
        ...
        <Box sx={{ display: "flex", fontSize: 14 }}>
          ...
          <Divider orientation="vertical" variant="middle" flexItem />
          <Typography
            sx={{ display: "flex", fontSize: 14, alignItems: "center", gap: 1 }}
          >
            <TableMenu />
          </Typography>
          <Divider orientation="vertical" variant="middle" flexItem />
