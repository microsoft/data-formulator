...
export const dataFormulatorSlice = createSlice({
  ...
  reducers: {
    ...
    queueChartEncodingUpdate: (
      ...
      const existingIndex = state.pendingEncodings.findIndex(
        (entry) =>
      ...
      if (existingIndex >= 0) {
        state.pendingEncodings[existingIndex] = action.payload;
      } else {
        state.pendingEncodings.push(action.payload);
      }
    },
    applyAllPendingEncodings: (state) => {
      state.pendingEncodings.forEach(
        ({ chartId, channel, prop, value, encoding }) => {
          const chart = dfSelectors
            .getAllCharts(state)
            .find((c) => c.id == chartId);
          if (!chart) return;
          ...
          const enc = chart.encodingMap[channel];
          if (!enc) return;
          if (!prop) {
            return;
          }
          ...
          switch (prop) {
            case "fieldID": {
              ...
              const table = state.tables.find((t) => t.id == chart.tableRef);
              if (
                table &&
                field &&
                table.metadata?.[field.name]?.levels?.length > 0
              ) {
                enc.sortBy = JSON.stringify(table.metadata[field.name].levels);
              }
              break;
            }
            case "aggregate":
            ...
            case "stack":
              ...
              break;
            case "sortOrder":
              enc.sortOrder = value == "auto" ? undefined : value;
              break;
            case "sortBy":
              enc.sortBy = value == "auto" ? undefined : value;
              break;
            case "scheme":
              enc.scheme = value;
              break;
...
export const dfSelectors = {
  getActiveModel: (state: DataFormulatorState): ModelConfig => {
    return (
      state.models.find((m) => m.id == state.modelSlots.generation) ||
      state.models[0]
    );
  },
  getModelBySlot: (
