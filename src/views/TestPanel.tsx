...
> {
  ...
  private testStreamingChart = () => {
    ...
    if (!import.meta.env.VITE_API_HOST || !import.meta.env.VITE_API_PORT) {
      console.warn(
        "VITE_API_HOST hoặc VITE_API_PORT chưa được thiết lập. Đang dùng giá trị mặc định:",
        apiHost,
        apiPort
      );
    } else {
      console.info("API endpoint:", apiHost, apiPort);
    }
    fetch(`http://${apiHost}:${apiPort}/stream`)
