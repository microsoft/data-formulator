...
export const ConceptGroup: FC<{ groupName: string; fields: FieldItem[] }> =
  function ConceptGroup({ groupName, fields }) {
    ...
    // Khi expanded = false, không hiển thị field nào
    const displayFields = expanded ? fields : [];
    ...
    return (
      <Box>
        ...
        {/* Vùng cuộn chỉ xuất hiện khi expanded */}
        {expanded && (
          <Box
            sx={{
              width: "100%",
    ...
    >
      ...
      <Box className="data-fields-group">
        <Box
          ...
          sx={{ height: "calc(100vh - 120px)", overflowY: "auto" }}
        >
          <Box sx={{ display: "block", width: "100%" }}>
            <Divider orientation="horizontal" textAlign="left">
              <Typography
          ...
          {groupNames.map((groupName) => {
            let fields = conceptItemGroups
              .filter((g) => g.group == groupName)
              .map((g) => g.field);
            fields = fields.sort((a, b) => {
              if (
                focusedTable &&
                focusedTable.names.includes(a.name) &&
                !focusedTable.names.includes(b.name)
              ) {
                return -1;
              } else if (
                focusedTable &&
  ...
  return (
    <Box
      sx={{
        ...
        borderLeftColor: conceptPanelOpen
          ? "transparent"
    ...
    >
      <Tooltip
        placement="left"
        title={conceptPanelOpen ? "hide concept panel" : "open concept panel"}
      >
        <IconButton
          color="primary"
          sx={
      ...
      <Box
        onClick={() =>
          !conceptPanelOpen && setConceptPanelOpen(!conceptPanelOpen)
        }
        sx={{
          overflow: "hidden",
          "&::after": conceptPanelOpen
            ? undefined
            : {
                content: '""',
                position: "absolute",
