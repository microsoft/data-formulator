...
export const ModelSelectionButton: React.FC<{}> = ({}) => {
      ...
      <Box sx={{ mb: 2 }}>
        ...
        <Box sx={{ display: "flex", gap: 1.5, mb: 2, flexWrap: "wrap" }}>
          {slotTypes.map((slotType) => {
            ...
            const assignedModel = assignedModelId
              ? models.find((m) => m.id === assignedModelId)
              : undefined;
            ...
            if (slotType == "generation") {
              ...
              modelExplanation =
                "background data type inference, code explanation";
            }
            ...
            return (
              <Box
                ...
                sx={{
                  ...
                  backgroundColor:
                    assignedModel && getStatus(assignedModelId) == "ok"
                      ? alpha(theme.palette.success.main, 0.05)
                      : assignedModel && getStatus(assignedModelId) == "error"
                  ...
                  borderColor:
                    assignedModel && getStatus(assignedModelId) == "ok"
                      ...
                      : getStatus(assignedModelId) == "error"
              ...
              >
                ...
                <Typography
                  ...
                  sx={{ fontSize: "0.75rem", mt: 0.5, mb: 1 }}
                >
                  - {modelExplanation}
                </Typography>
                  ...
                  >
                    ...
                    {models.map((model) => (
                      <MenuItem
                        key={model.id}
                        value={model.id}
                        disabled={getStatus(model.id) !== "ok"}
                      >
                        <Box
                          sx={{
                            display: "flex",
                        ...
                        >
                          ...
                          {getStatus(model.id) === "ok" ? (
                            <CheckCircleOutlineIcon
                              sx={{ color: "success.main", ml: 1 }}
                              fontSize="small"
                            />
                          ) : getStatus(model.id) === "error" ? (
                            <ErrorOutlineIcon
                              sx={{ color: "error.main", ml: 1 }}
                              fontSize="small"
                            />
        ...
        >
          <strong>💡 Tip:</strong> Use powerful models for generation tasks and
          faster models for hints.
        </Typography>
      </Box>
    );
  };
      ...
      >
        ...
        <TableBody>
          {models.map((model) => {
            ...
            const isAssignedToAnySlot = dfSelectors
              .getAllSlotTypes()
              .some((slotType) => isModelAssignedToSlot(model.id, slotType));
              ...
              <React.Fragment key={`${model.id}`}>
                <TableRow
                  ...
                  sx={{
                    ...
                    backgroundColor: isAssignedToAnySlot
          ...
          <TableRow>
            <TableCell colSpan={8} sx={{ pt: 2, pb: 1 }}>
              ...
              >
                <strong>Configuration:</strong> Based on LiteLLM.{" "}
                <a
  ...
  let notAllSlotsReady =
    Object.values(tempModelSlots).filter((id) => id).length !==
      dfSelectors.getAllSlotTypes().length ||
    Object.values(tempModelSlots)
      .filter((id) => id)
      .some((id) => getStatus(id) !== "ok");

  let genModelName =
    models.find((m) => m.id == modelSlots["generation"])?.model || "Unknown";
  let hintModelName =
    models.find((m) => m.id == modelSlots["hint"])?.model || "Unknown";
  let modelNames =
    genModelName == hintModelName
      ? genModelName
      : `${genModelName} / ${hintModelName}`;
    ...
    <>
      <Tooltip title="Configure model assignments for different task types">
        <Button
          ...
          variant="text"
          color={notAllSlotsReady ? "warning" : "primary"}
          onClick={() => {
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            disabled={notAllSlotsReady}
            variant={
              Object.values(tempModelSlots).filter((id) => id).length == 0
                ? "text"
                : "contained"
            }
            onClick={() => {
