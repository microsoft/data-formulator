...
export const deriveTransformExamplesV2 = (code: string, parentIDs: string[], inputListLength: number, 
    ...
    const parentDomains = parentConcepts.map(concept => {
        if (concept.source == "derived") {
            const transform = concept.transform as ConceptTransformation;
            const domain = deriveTransformExamplesV2(
    ...
    const examplesFromEachParent = inputListLength > 0 ? parentDomains.map(l => l.slice(0, inputListLength)) : parentDomains;
   
    const maxValueLength = examplesFromEachParent.length > 0 ? Math.min(...examplesFromEachParent.map(l => l.length)) : 0;

    const inputTupleList = [...Array(maxValueLength).keys()].map(i => examplesFromEachParent.map(exampleList => exampleList[i]));
...
export const processCodeCandidates = (rawCodeList: string[], parentIDs: string[], conceptShelfItems: FieldItem[], tables: DictTable[]) : string[] => {
    ...
    const tempCodeList = rawCodeList.filter(code => {
        try {
    ...
    const isArrayOrObj = (a: any) => {
        return ((!!a) && (a.constructor === Array)) || (!!a) && (a.constructor === Object);
    }
    const codeOutputSignatures = tempCodeList.map(codeStr => {
            const result = [codeStr, deriveTransformExamplesV2(codeStr, parentIDs, -1, conceptShelfItems, tables)] as [string, [any[], any][]]
            return result
        })
    ...
    for (let i = 0; i < codeOutputSignatures.length; i ++) {
        const [codeStr, signature] = codeOutputSignatures[i];
