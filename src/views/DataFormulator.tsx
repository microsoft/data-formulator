...
export const DataFormulatorFC = ({}) => {
  const displayPanelSize = useSelector(
    (state: DataFormulatorState) => state.displayPanelSize
  );
  const visPaneSize = useSelector(
    (state: DataFormulatorState) => state.visPaneSize
  );
  ...
  const visPane = // @ts-ignore
    (
      <SplitPane
        ...
        onDragFinished={(size) => {
          dispatch(dfActions.setVisPaneSize(size));
        }}
      >
        {visPaneMain}
  ...
  const splitPane = // @ts-ignore
    (
      <MainSplitPane
        ...
        onDragFinished={(size) => {
          dispatch(dfActions.setDisplayPanelSize(size));
        }}
      >
  ...
  const fixedSplitPane = (
    <Box sx={{ display: "flex", flexDirection: "row", height: "100%" }}>
      ...
      <Box
        ...
        sx={{ width: 280, borderLeft: "1px solid lightgray" }}
      >
        {conceptEncodingPanel}
        {/* <InfoPanelFC $tableRef={$tableRef}/> */}
      </Box>
    ...
    <Box sx={{ width: "100vw" }}>
      ...
      >
        ...
        <Typography variant="h4">
          ...
          <TableUploadDialog buttonElement={"file"} disabled={false} />, or{" "}
          <TableCopyDialogV2 buttonElement={"clipboard"} disabled={false} />
        </Typography>
        <Typography sx={{ width: 960, margin: "auto" }} variant="body1">
          ...
          <Tooltip
            title={
              <Box>
                Example of a messy text block:{" "}
                <Typography
                  sx={{ fontSize: 10, marginTop: "6px" }}
                  component={"pre"}
                >
                  {exampleMessyText}
                </Typography>
          ...
          <Tooltip
            title={
              <Box>
                Example of a table in image format:{" "}
                <Box
                  component="img"
                  sx={{ width: "100%", marginTop: "6px" }}
                  alt=""
                  src={exampleImageTable}
                />
              </Box>
            }
          >
          ...
          that contain data into clipboard to get started.
        </Typography>
      </Box>
      <Button
        size="small"
    ...
    <Box sx={{ width: "100vw" }}>
      ...
      >
        ...
        <Typography variant="body1">
          Specify an OpenAI or Azure OpenAI endpoint to run {toolName}.
        </Typography>
      </Box>
      <Button
        size="small"
