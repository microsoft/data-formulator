...
import { AssembleOptions, ChartWarning } from '../lib/agents-chart';
...
// ------ Pyramid Chart ------
function genPyramidTests(): TestCase[] {
    const tests: TestCase[] = [];
    const rand = seededRandom(777);
    ...
    // 2. Workforce pyramid — Grade × Full-Time / Part-Time headcount
    {
    ...
    // 3. Survey responses — Satisfaction Level × Agree / Disagree
    {
    ...
    // 4. Income bracket pyramid — many bands (medium)
    {
    ...
    // 5. Education pyramid — Degree Level × Admitted / Rejected
    {
    ...
    // 6. Country comparison — Country × Import / Export value
    {
    ...
    // 7. Large cardinality — 20 age bands (overstretch test)
    {
...
// ============================================================================
// Date Format Stress Tests
// ============================================================================

/**
 * Generates a comprehensive set of date-format stress tests.
 * Each date format is tested across multiple chart types to ensure
 * temporal detection, axis rendering, and mark sizing work correctly.
 */
// ---------------------------------------------------------------------------
...
function genDateTests(dateFormats: DateFormat[], seed: number): TestCase[] {
    const tests: TestCase[] = [];
    const rand = seededRandom(seed);
...
// ---------------------------------------------------------------------------
function genDateYearTests(): TestCase[] {
    return genDateTests([
        {
            label: 'Year (string)',
...
// 2. Month only
// ---------------------------------------------------------------------------
function genDateMonthTests(): TestCase[] {
    return genDateTests([
        {
            label: 'Month (English)',
...
// 3. Year-Month
// ---------------------------------------------------------------------------
function genDateYearMonthTests(): TestCase[] {
    return genDateTests([
        {
...
// 4. Decade
// ---------------------------------------------------------------------------
function genDateDecadeTests(): TestCase[] {
    return genDateTests([
        {
            label: 'Decade (XXXXs)',
...
// 5. Date / DateTime
// ---------------------------------------------------------------------------
function genDateDateTimeTests(): TestCase[] {
    return genDateTests([
        {
...
// ---------------------------------------------------------------------------
function genDateHoursTests(): TestCase[] {
    return genDateTests([
        {
        ...
        try {
            ...
            for (const [ch, ei] of Object.entries(testCase.encodingMap)) {
                if (ei && ei.fieldID) {
                    ...
                    if (ei.stack) entry.stack = ei.stack;
