...
export const FreeDataViewFC: FC<FreeDataViewProps> = function DataView() {
  ...
  let derivedFields = conceptShelfItems.filter(
    (f) => f.source == "derived" && f.name != "",
  );
  ...
  let tableToRender = useMemo(() => {
    if (derivedFields.some((f) => f.tableRef == focusedTableId)) {
      return tables.map((table) => {
        ...
        let extTable = createTableFromFromObjectArray(
          `${table.id}`,
          rows,
          table.anchored,
          table.derive,
  ...
  let coreTables = tableToRender.filter(
  ...
  return (
    ...
    >
      <Box sx={{ display: "flex" }}>
        <Typography
          sx={{ display: "flex", color: "rgba(0,0,0,0.5)", ml: 1 }}
          component="span"
        >
