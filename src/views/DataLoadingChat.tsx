...
import {
  createOrderedThreadBlocks,
  DataLoadingInputBox,
  DataPreviewBox,
  SingleDataCleanThreadView,
} from "./DataLoadingThread";
...
export const DataLoadingChat: React.FC<DataLoadingChatProps> = ({
  ...
  if (!existOutputBlocks && !streamingContent) {
    return (
      <Box sx={{ width: "calc(100% - 32px)", borderRadius: 2, px: 2 }}>
        <DataLoadingInputBox
          maxLines={24}
  ...
  let chatCard = (
    <Box
      sx={{
        width: existOutputBlocks || streamingContent ? "960px" : "640px",
        minHeight: 400,
        display: "flex",
        flexDirection: "row",
        borderRadius: 2,
          ...
          >
            {selectedTable ? (
              ...
              <Typography
                variant="body2"
                color="text.secondary"
                sx={{ textAlign: "center", mt: 4, fontSize: "11px" }}
              >
                No data available
              </Typography>
            )}
            ...
            >
              <Button
                ...
                disabled={
                  !selectedTable ||
                  selectedTable.content.type !== "image_url" ||
                  cleanInProgress
                }
                size="small"
              >
              ...
              <Button
                ...
                disabled={
                  !selectedTable || selectedTable.content.type !== "csv"
                }
                size="small"
              >
                Load table
    ...
    <>
      ...
      >
        ...
        <DialogContent sx={{ overflowX: "hidden", padding: 1 }}>
          <DataLoadingChat onLoadComplete={() => setDialogOpen(false)} />
        </DialogContent>
      </Dialog>
    </>
  );
