...
export const EncodingBox: FC<EncodingBoxProps> = function EncodingBox({
  ...
  const encoding = React.useMemo(() => {
    let result = baseEncoding ? { ...baseEncoding } : ({} as EncodingItem);
    pendingEncodingsForChannel.forEach((entry) => {
      if (entry.encoding !== undefined) {
        result = { ...entry.encoding };
      } else if (entry.prop !== undefined) {
        result = { ...result, [entry.prop]: entry.value };
      }
    });
    return result;
  }, [baseEncoding, pendingEncodingsForChannel]);

  const dispatch = useDispatch();
  const queueEncodingUpdate = (params: {
    prop?: string;
    value?: any;
    encoding?: EncodingItem;
  }) => {
