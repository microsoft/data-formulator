...
### 2.6 `connectors/` — Data Connector 配置（用户级）

**位置**: `users/<identity>/connectors/<source_id>.json`（不在 workspace 内）
...
### 2.7 `catalog_cache/` — 数据源 Catalog 元数据快照（用户级）

**位置**: `users/<identity>/catalog_cache/<source_id>.json`（不在 workspace 内）
