...
| `sync_catalog_metadata(table_filter=None)` | 实例 | 全量 metadata sync；默认调用 `list_tables()` + `ensure_table_keys()`。需要额外 API 调用获取列信息时 override（如 Superset） |
...
- [ ] 若 `list_tables()` 不含完整列信息，override `sync_catalog_metadata()` 添加并发列获取（参考 `SupersetLoader`）。
