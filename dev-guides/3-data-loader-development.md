...
- `list_tables` 返回的 metadata 应尽量包含 `row_count`、`columns`、`sample_rows`，保证前端行为一致。
...
- 表/列描述是接口层可承载的可选元数据，不是所有内置 Loader 都已完整填充。当前导入链路主要写入 Arrow 推断的列名/类型和 `source_info`，尚未统一把源系统表/列描述合并进 Workspace。
