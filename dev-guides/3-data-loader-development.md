...
| `search_catalog(query, limit=100)` | instance | Cross-level catalog search; default falls back to `list_tables(table_filter=query)`. |
...
- Large catalogs should override `search_catalog()` for lightweight metadata search; the frontend calls `/api/connectors/search-catalog` only on Enter/search button.
