...
> **核心原则**: 应用可控错误一律 HTTP 200，仅认证/授权和不可控传输错误使用非 200

## 1. 核心契约

DF 的错误传递以 **body `status` 字段为唯一判据**。HTTP 状态码仅用于传输层信号。

| 层级 | 规范 |
|------|------|
| HTTP 状态码 | 应用错误 → `200`；认证错误 → `401`/`403`；不可控错误 → `404`/`413`/`500` |
| 应用层 body | 成功 → `"status": "success"`；失败 → `"status": "error"` |
...
| HTTP 状态码 | 使用场景 | 由谁返回 |
|-------------|----------|----------|
| `200` | 所有应用可控响应（成功与业务错误） | `json_ok()` / `_handle_app_error()` |
...
- 前端可统一通过 `body.status === "error"` 检测错误
...
# 带过渡字段（token 将在后续清理中移除）
return json_ok(data, token=token)
...
- `AppError` 路径断言 HTTP 200 + body `status == "error"` + `error.code` 匹配
