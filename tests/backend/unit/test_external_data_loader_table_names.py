...
@pytest.mark.xfail(
    reason="Known issue: external loader sanitization still uses an ASCII-only allowlist",
    strict=False,
)
...
@pytest.mark.xfail(
    reason="Known issue: external loader sanitization still removes Chinese characters around separators",
    strict=False,
)
...
@pytest.mark.xfail(
    reason="Known issue: external loader sanitization still drops Chinese characters for digit-prefixed names",
    strict=False,
)
