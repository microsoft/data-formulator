...
class TestMigrateRoute:
    def test_migrate_moves_and_cleans_source(self, client):
        ...
        with (
            patch("data_formulator.session_routes.get_identity_id", return_value="user:alice"),
            patch(
                "data_formulator.session_routes.get_workspace_manager",
    ...
    def test_migrate_rejects_non_user(self, client):
        with patch("data_formulator.session_routes.get_identity_id", return_value="browser:abc"):
            resp = client.post(
                "/api/sessions/migrate",
                json={"source_identity": "browser:xyz"},
            )
...
class TestCleanupAnonymousRoute:
    def test_cleanup_success(self, client):
        ...
        with (
            patch("data_formulator.session_routes.get_identity_id", return_value="user:alice"),
            patch("data_formulator.session_routes.get_workspace_manager", return_value=source_mgr),
        ):
            resp = client.post(
