...
def run_exploration_flow_streaming(
    ...
    exploration_agent = ExplorationAgent(client, db_conn=db_conn)
    # only for first step exploration when the user doesn't come up with a plan
    interactive_explore_agent = InteractiveExploreAgent(client, agent_exploration_rules=agent_exploration_rules, db_conn=db_conn)
    ...
    if len(current_plan) == 0:
        ...
        try:
            ...
            # Add initial plan generated by interactive_explore_agent
            exploration_plan_list.append({
                "ref_tables": [{"name": table['name'], "rows": table['rows'][:5] if 'rows' in table else []} for table in input_tables],
