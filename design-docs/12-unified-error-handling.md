...
### Phase 2: 后端路由改造 — 高优先级端点

**目标**: 改造最常触发错误的流式端点和关键非流式端点。

#### Step 2.1: 流式端点统一（5 个）
- [ ] `/data-agent-streaming` → 错误行用 `stream_error_event()`
- [ ] `/get-recommendation-questions` → 去掉 `error: ` 前缀，用 `stream_error_event()`
- [ ] `/generate-report-chat` → 去掉 `data: ` 前缀，统一为 NDJSON
- [ ] `/data-loading-chat` → 用 `classify_and_wrap_llm_error()` 替换 `str(e)`
- [ ] `/clean-data-stream` → 用 `stream_error_event()`

#### Step 2.2: 关键非流式端点
- [ ] `/process-data-on-load` → 增加 `error` 字段
- [ ] `/sort-data` → 使用 `AppError`
- [ ] `/derive-data` + `/refine-data` → 使用 `AppError`
- [ ] `/test-model` → 使用 `AppError`

#### Step 2.3: 移除 `agent_bp.errorhandler(Exception)`
- [ ] 由全局处理器接管（`register_error_handlers` 已覆盖）

#### Step 2.4: 修复安全漏洞
- [ ] `data-loading-chat` 的 `str(e)` → `classify_and_wrap_llm_error(e)`
- [ ] `data_connector.py` 中 `str(e)` → `sanitize_error_message`

**Phase 2 为每个端点编写集成测试**（Flask test_client 模拟错误场景）。

#### Step 2.5: 更新 Cursor Rules

- [ ] `.cursor/rules/error-response-safety.mdc` — 补充已改造端点的具体示例
  - 添加流式端点 `stream_error_event()` 的真实代码片段
  - 标注哪些端点已完成改造，作为参照

- [ ] `.cursor/rules/unified-error-protocol.mdc` — 补充流式协议迁移注意事项
  - 记录 `generate-report-chat` 从 `data: ` 前缀迁移到纯 NDJSON 的变更
  - 记录 `get-recommendation-questions` 从 `error: ` 前缀迁移的变更
...
#### Step 3.1: 流式调用点（与 Phase 2 配合）
- [ ] `SimpleChartRecBox.tsx` — `data-agent-streaming` 解析逻辑
- [ ] `SimpleChartRecBox.tsx` — `get-recommendation-questions` 解析逻辑
- [ ] `SimpleChartRecBox.tsx` — `generate-report-chat` 解析逻辑
- [ ] `DataLoadingChat.tsx` — `data-loading-chat` 解析逻辑
- [ ] `useFormulateData.ts` — `get-recommendation-questions` + `derive-data`

#### Step 3.2: 高频空 catch（每个改为 `handleApiError`）
- [ ] `App.tsx:957` — `APP_CONFIG` 加载失败提示
- [ ] `DataSourceSidebar.tsx` — 4 处空 catch
- [ ] `VisualizationView.tsx:556` — `sample-table`
- [ ] `DataFormulator.tsx:100` — connectors
- [ ] `UnifiedDataUploadDialog.tsx` — 3 处空 catch

#### Step 3.3: RTK thunk rejected 处理
- [ ] `fetchFieldSemanticType.rejected` → `addMessages`
- [ ] `fetchCodeExpl.rejected` → `addMessages`
- [ ] `fetchChartInsight.rejected` → `addMessages`（替换 `console.error`）

#### Step 3.4: 裸 fetch 修复
- [ ] `ModelSelectionDialog.tsx:296` → 改用 `fetchWithIdentity`

#### Step 3.5: 更新 Cursor Rules

- [ ] `.cursor/rules/frontend-test-conventions.mdc` — 补充 API 错误测试示例
  - 添加 mock `apiRequest` / `streamRequest` 返回错误的测试模式
  - 添加验证 `addMessages` 被调用的断言示例

- [ ] `.cursor/rules/unified-error-protocol.mdc` — 补充前端改造完整示例
  - 添加 RTK thunk rejected 处理的标准写法
  - 添加组件内流式消费 + 错误处理的标准写法
  - 添加"禁止事项"：空 catch、裸 fetch、console.error-only 等
...
- [ ] 用 `AppError` 替换 `sanitize_db_error_message` 的返回方式
- [ ] `list-tables` 内部降级逻辑保留，但记录 warning 级 Message
...
- [ ] 逐一审查 `agent_utils.py`、`demo_stream.py` 等
- [ ] 区分"合理降级" vs "应该传播"
- [ ] 合理降级的添加 `logger.warning`

#### Step 4.5: 更新 Cursor Rules & Skills — 最终版

全部改造完成后，将 rules 和 skill 更新为"终态"版本：

- [ ] `.cursor/rules/error-response-safety.mdc` — **最终版**
  - 移除对旧 `safe_error_response` 的引用（已无调用方）
  - 完整示例只包含 `AppError` / `stream_error_event` 方式
  - Common Mistakes 更新为新体系下的典型错误

- [ ] `.cursor/rules/unified-error-protocol.mdc` — **最终版**
  - 完整的 ErrorCode 清单
  - 后端非流式 / 流式 / Agent 层的标准错误处理模板
  - 前端 `apiRequest` / `streamRequest` / `handleApiError` 完整用法
  - 新增端点的 checklist（必须包含错误处理 + 测试 + i18n）

- [ ] `.cursor/skills/error-handling/SKILL.md` — **最终版**
  - 完整的端到端错误处理开发指南
  - 涵盖："新增后端端点"、"新增前端 API 调用"、"新增错误码"、"调试错误传递链路" 四个场景
  - 包含真实代码示例（从已改造的端点中提取）

- [ ] `.cursor/rules/backend-test-conventions.mdc` — 补充错误测试约定
  - 新路由必须包含错误场景测试（至少覆盖 AppError、未知异常两种）
  - 流式端点测试必须验证错误事件行格式
