...
### 5.1 后端：HTTP 语义化状态码 + 统一 JSON envelope

所有普通 `/api/` JSON endpoint 采用 **HTTP 状态码 + body 双层信号**：

- HTTP 状态码承载 transport 语义（devtools 可见、proxy/CDN 友好）
- body 内 `status` 字段保留自描述性（日志可搜、合约断言可用）
- 两者始终一致：2xx → `status:"success"`，4xx/5xx → `status:"error"`
...
**ErrorCode → HTTP 状态码映射表：**

| ErrorCode | HTTP Status | 语义 |
|-----------|-------------|------|
| _(success)_ | 200 | 请求成功 |
| `INVALID_REQUEST` | 400 | 请求参数缺失/格式错误 |
| `VALIDATION_ERROR` | 422 | 业务校验不通过（如图表不可分析） |
| `AGENT_ERROR` | 422 | LLM 输出解析失败、candidates 为空等业务失败 |
| `AUTH_REQUIRED` / `AUTH_EXPIRED` | 401 | 需要认证 |
| `ACCESS_DENIED` | 403 | 权限不足 |
| `RATE_LIMITED` | 429 | LLM 提供商限流 |
| `CONNECTOR_ERROR` | 502 | 外部数据源连接失败 |
| `SERVICE_UNAVAILABLE` | 503 | LLM 提供商不可达 |
| `INTERNAL_ERROR` | 500 | 未预期服务端异常 |

**设计原则**：
- 4xx = 客户端能通过修正输入/行为解决的问题
- 5xx = 客户端无法解决，需等待/重试
- 422 用于"请求格式没问题，但业务逻辑无法完成"（RFC 9110 推荐）
- `retry` 字段是对 HTTP 状态码的补充——同为 5xx，`RATE_LIMITED` 可重试但 `INTERNAL_ERROR` 不一定
...
- 前端 `apiRequest()` 通过 `response.ok` 做第一道成功/失败判断，不再依赖 body 内 `status` 字段做分流。
- 前端 `parseApiResponse()` 删除 `body.error_message ?? body.message` 回退分支和 `body.result` 回退分支。
...
1. **流建立前校验失败**（参数缺失、格式错误、认证失败等）：此时还没开始 yield NDJSON 行，可以直接返回 HTTP 4xx + 普通 JSON error body（`Content-Type: application/json`），与普通 JSON endpoint 的错误格式完全一致。前端通过 `!response.ok` 或 `Content-Type` 判断走普通错误解析还是 NDJSON 流解析。
...
>
> - `dev-guides/1-streaming-protocol.md` § 1 当前写"流式端点始终返回 200"，需修改为"流建立后始终 200；流建立前校验失败返回 HTTP 4xx + JSON error"。
> - `dev-guides/7-unified-error-handling.md` § 3.1 当前示例返回 HTTP 200 JSON，需修改为返回 HTTP 4xx。
> - 以上两处变更在 Phase 1c 规范更新时一并完成。

### 5.2 后端：提供 `json_ok()` / `json_error()` helper + 全局 error handler

在新模块 `py-src/data_formulator/api_response.py` 中提供两个 helper：

```python
from flask import jsonify, current_app, Response

def json_ok(data: dict) -> Response:
    """成功响应 (HTTP 200)。业务数据必须显式传入 dict，包裹在 data 字段内。"""
    return jsonify({"status": "success", "data": data})

def json_error(error: AppError) -> tuple[Response, int]:
    """显式错误响应 (HTTP 4xx/5xx)。返回 (response, http_status) tuple。
    推荐直接 raise AppError，全局 handler 自动处理。此函数仅用于少量不适合抛异常的显式分支。"""
    body = {"status": "error", "error": error.to_dict(include_detail=current_app.debug)}
    return jsonify(body), error.http_status
```

`json_ok()` 只接受一个 `data: dict` 参数，不提供 `**kwargs` 便捷接口——避免"两种传入方式导致覆盖冲突"的 footgun。调用方必须显式构造 dict：
...
**全局 error handler（HTTP 状态码自动映射）**：

```python
ERROR_CODE_TO_HTTP: dict[str, int] = {
    ErrorCode.INVALID_REQUEST: 400,
    ErrorCode.VALIDATION_ERROR: 422,
    ErrorCode.AGENT_ERROR: 422,
    ErrorCode.AUTH_REQUIRED: 401,
    ErrorCode.AUTH_EXPIRED: 401,
    ErrorCode.ACCESS_DENIED: 403,
    ErrorCode.RATE_LIMITED: 429,
    ErrorCode.CONNECTOR_ERROR: 502,
...
`AppError` 类新增 `http_status` 属性（从 `ERROR_CODE_TO_HTTP` 查表或构造时指定），确保 `raise AppError(...)` 和 `json_error(...)` 两种路径返回相同的 HTTP 状态码。

推荐业务代码直接 `raise AppError(...)`，由全局 handler 处理。`json_error()` 只用于少量不适合抛异常的显式分支（如需要在响应前做额外清理）。
...
|------|-----------|-------------|-------|
| candidates 为空 | `AGENT_ERROR` | 422 | false |
| LLM 输出无法解析 | `AGENT_ERROR` | 422 | false |
| LLM API timeout / rate limit / auth | `classify_and_wrap_llm_error()` | 429/503 | 按分类 |
| 缺少必要参数 | `INVALID_REQUEST` | 400 | false |
| 图表不可分析 / 无图像 | `VALIDATION_ERROR` | 422 | false |
| 模型不支持 vision 但请求包含图片 | `VALIDATION_ERROR` | 422 | false |
| `chart_image` 为空字符串 | `VALIDATION_ERROR` | 422 | false |
...
**`apiRequest()` 改为 `response.ok` 优先判断**：

迁移完成后，`apiClient.ts` 中的 `apiRequest()` 以 HTTP 状态码作为第一道判断：
...
export async function apiRequest<T = any>(
    ...
    if (!response.ok) {
        // HTTP 4xx/5xx — body 一定是 error envelope
    ...
    // HTTP 2xx — body 一定是 success envelope
    return { data: body.data as T };
}
```

关键改进：
- `response.ok`（HTTP 200-299）作为成功信号，不再依赖 body 内 `status` 字段做分流
- 返回类型简化为 `{ data: T }`，不再携带 `token`
- 对 4xx/5xx 但 body 不符合 error envelope 的情况，构造 fallback `ApiError`（`retry` 默认 5xx 可重试）
...
`parseApiResponse()` 作为 `apiRequest()` 的低层辅助，仅在 `streamRequest()` pre-stream 判断中单独使用：
...
export function parseApiResponse<T = any>(
    ...
    if (httpStatus >= 400 || body.status === 'error') {
        if (!body.error || typeof body.error !== 'object' || !body.error.code) {
            throw new ApiRequestError(
                { code: 'PROTOCOL_VIOLATION', message: 'Invalid error envelope from server', retry: false },
                httpStatus,
            );
        }
...
收到 HTTP 4xx/5xx 但无 `error.code` 的响应时，抛出 `PROTOCOL_VIOLATION` 而不是静默降级，以暴露未迁移的后端 endpoint。
...
1. invalid request 必须返回 HTTP 400 + `status: "error"`。
2. 业务失败必须返回 HTTP 422 + `status: "error"`。
3. 成功响应必须返回 HTTP 200 + `status: "success"` 和 `data`。
4. LLM / connector / DB exception 必须返回对应 HTTP 4xx/5xx + 结构化 `error.code`，不能暴露原始异常文本。
5. 所有响应必须包含 `X-Request-Id` header。
6. 响应 body 中不得包含 `token` 字段。

对 chart insight 的测试应至少覆盖：

- 缺少 JSON body → HTTP 400 + `INVALID_REQUEST`。
- `ChartInsightAgent.run()` 返回 `[]` → HTTP 422 + `AGENT_ERROR`。
- `ChartInsightAgent.run()` 返回 `{"status":"other error","content":"..."}` → HTTP 422 + `AGENT_ERROR`。
- `ChartInsightAgent.run()` 抛出 timeout/rate limit 类异常 → HTTP 429/503。
...
1. 后端返回 HTTP 4xx/5xx + `{status:"error", error:{...}}` 时 thunk rejected 并携带结构化 `ApiRequestError`。
...
目标：消除"静默停止"，全量迁移所有 `/api/` JSON endpoint 到 HTTP 语义化状态码 + 统一 envelope，移除 `token` 字段，建立测试 + 静态扫描 + ESLint 三层护栏。

#### Phase 1a：后端全量迁移

- 新增 `json_ok()` / `json_error()` helper（`json_error()` 返回 `tuple[Response, int]`）。
- 新增全局 `ERROR_CODE_TO_HTTP` 映射表 + `handle_app_error` handler（参见 5.2），确保 `raise AppError(...)` 自动返回正确的 HTTP 状态码。
...
- 所有错误响应统一为 HTTP 4xx/5xx + `{"status": "error", "error": {"code": "...", "message": "...", "retry": false}}`，HTTP 状态码根据 `ErrorCode` 自动映射（参见 5.1 映射表）。
...
- `/chart-insight` 特别处理：
  - 新增 vision 能力校验：`model_supports_vision(model) == False` 且 `chart_image` 非空时返回 `VALIDATION_ERROR`（HTTP 422）+ 明确提示切换模型。
  - 新增图片有效性校验：`chart_image` 为空字符串时返回 `VALIDATION_ERROR`（HTTP 422）+ 提示重试。
  - candidates 空 / `status != "ok"` 映射为 `AGENT_ERROR`（HTTP 422）；exception 使用 `classify_and_wrap_llm_error()`。
...
- `client_utils.py` 中的 `reasoning_effort` 从硬编码改为参数化（接受 agent 调用时传入，默认 `"low"`），为未来 per-agent 配置留出接口。
...
- **`reasoning_effort` 参数化**：`get_completion()` 接受 `reasoning_effort` 参数，Agent 调用时传入（如 `"low"` / `"medium"` / `"high"`），不在 `client_utils.py` 内按 model name 硬编码。
...
> **前置决策（blocker）**：在开始 Phase 1b 前必须决定 Open Questions #1（snackbar vs inline）和 #2（默认开启 vs lazy generate），因为它们直接影响 rejected reducer 的实现方式和自动触发逻辑。

- 所有 LLM / Agent 用户可见 thunk（`fetchChartInsight`、`fetchCodeExpl`、`fetchFieldSemanticType`）改用 `apiRequest()`。
- `apiRequest()` 改为以 `response.ok` 做第一道判断（HTTP 4xx/5xx 直接 throw `ApiRequestError`），fulfilled reducer 不再需要检查 `action.payload.status`。
...
- `fetchCodeExpl` 和 `fetchFieldSemanticType` timeout 改用 `config.formulateTimeoutSeconds`。
- 修复 `fetchCodeExpl` 和 `fetchFieldSemanticType` 中缺失的 `clearTimeout(timeoutId)` 调用（当前是资源泄漏）。
...
- 收到 HTTP 4xx/5xx 但无 `error.code` 的响应时，抛出 `PROTOCOL_VIOLATION`，不再静默降级。
- 删除前端对裸 `{error: "..."}` 的兼容解析。
- 删除后端对请求 body 中 `token` 字段的读取代码（Phase 1a 只是忽略不报错，1c 彻底清理）。
- **整体重写** `dev-guides/7-unified-error-handling.md`：(1) § 1 核心契约从 "HTTP 200-only" 改为 "HTTP 语义化状态码"；(2) § 2.1 示例中 `"status": "ok"` 改为 `"status": "success"`；(3) § 2.2 legacy 兼容格式删除（不再标记 deprecated，直接移除）；(4) § 3.1 pre-stream 校验示例改为返回 HTTP 4xx；(5) § 4.2 前端消费规范更新为 `response.ok` 优先判断；(6) § 7 测试要求更新为断言 HTTP 4xx/5xx；(7) § 8 checklist 更新。
- 同步更新 `dev-guides/1-streaming-protocol.md`：(1) § 1 "始终返回 200" 改为"流建立后 200，流建立前校验失败返回 4xx"；(2) § 2.2 error 事件移除 `token` 字段；(3) § 5 端点对照表移除 `token`；(4) § 6 Checklist 更新。
- 更新 `.cursor/rules/unified-error-protocol.mdc`。
...
- 后端：所有 `/api/` JSON endpoint 的 contract tests（成功路径断言 HTTP 200 + `status:"success"` + `data`；失败路径断言 HTTP 4xx/5xx + `status:"error"` + `error.code`）。
- 后端：chart insight 特定测试（缺少 body → 400、candidates 为空 → 422、候选 status 非 ok → 422、LLM 异常 → 429/503、成功路径 → 200）。
- 后端：`X-Request-Id` header 出现在所有响应中。
- 前端：`apiRequest()` 对 HTTP 4xx/5xx 正确 throw `ApiRequestError`。
- 前端：`fetchChartInsight` error parsing tests。
- 前端：用户设置 180s 时不会被短硬编码超时截断。
- 前端：`parseApiResponse()` 对 legacy 格式抛出 `PROTOCOL_VIOLATION`（Phase 1c 之后）。
...
- 所有 `/api/` 普通 JSON endpoint 都通过合约测试和静态扫描。
- 所有 `/api/` 普通 JSON 成功响应都是 `status:"success"` + `data`。
- 所有 `/api/` 普通 JSON 失败响应都是 `status:"error"` + `error.code/message/retry`。
- 文件下载、NDJSON、redirect 等特例全部登记在豁免列表，并有测试说明。
...
- LLM / Agent 用户可见任务不存在未说明的 20/30/60 秒硬编码前端超时。
- （Phase 2）普通 chart insight 默认不发送 chart image；LLM 输入以结构化 profile 为主。
- （Phase 2）只有明确 fallback 场景会发送低分辨率图片，并在日志/diagnostics 中标记 `image_used=true`。
...
- 收到无 `error.code` 的错误响应时前端抛出 `PROTOCOL_VIOLATION`，而不是静默降级。
- `dev-guides/7-unified-error-handling.md` § 2.2 legacy 格式标记为 deprecated。
- 所有 `/api/` JSON 成功响应的 `body.status === "success"` 且业务数据在 `body.data` 中，无扁平散布。

## 10. Open Questions

1. **[Phase 1b blocker]** 自动洞察失败是否应该每次显示 snackbar，还是只在洞察 tab 打开时显示 inline error？
2. **[Phase 1b blocker]** 自动洞察是否应该默认开启，还是只在用户第一次打开洞察 tab 时 lazy generate？
