...
| `sync_catalog_metadata(table_filter=None)` | 实例 | 全量 metadata sync；默认调用 `list_tables()` + `ensure_table_keys()`。若 `list_tables()` 已包含列信息（如 Superset），则无需 override |
...
- `list_tables` 返回 catalog metadata（含列信息为佳）。部分 Loader（如 Superset）在 `list_tables()` 中并行拉取列信息，使首次连接即写入完整 cache。不要为了列表页或 Agent 缓存读取样本行或逐表计数。
...
| `MSSQLDataLoader` | SQL Server | `pyodbc`；支持 SQL 和 Windows 认证 |
| `AzureSQLDataLoader` | Azure SQL | 继承 MSSQL 数据平面；要求目标数据库和委托 Microsoft Entra access token，并强制 ODBC Driver 18/TLS 安全默认值 |
...
- [ ] Azure SQL 委托模式通过共享 MSSQL 数据平面，以
    `SQL_COPT_SS_ACCESS_TOKEN` 传入 UTF-16-LE 编码的 length-prefixed
    token；通用 `mssql` connector 不公开委托登录，连接串不得同时包含
    `UID`、`PWD`、`Trusted_Connection` 或 `Authentication`。

#### 数据与 catalog

- [ ] `list_tables()` 返回轻量 metadata，不做逐表采样、计数或大数据读取。
- [ ] 每条 table 记录必须含 `table_key` 字段——该数据源中的稳定唯一标识（如 Superset UUID、SQL `_source_name`）。基类 `ensure_table_keys()` 提供 fallback，但 Loader 应主动设置。
- [ ] 若 `list_tables()` 不含完整列信息，在 `list_tables()` 中添加并行列拉取（参考 `SupersetLoader._enrich_columns()`），或 override `sync_catalog_metadata()` 补充列获取。
- [ ] 若 `information_schema` 是单库隔离的（PostgreSQL、MSSQL），override `sync_catalog_metadata()` 在 `database` 为空时遍历所有可访问数据库。
...
- [ ] `sample_rows` 等面向前端的 DataFrame→records 转换使用 `df_to_safe_records(df)` 而非 `to_json`/`to_dict`（详见 `docs/dev-guides/15-dataframe-serialization.md`）。
- [ ] `size` 必须通过 `min(opts.get("size", MAX_IMPORT_ROWS), MAX_IMPORT_ROWS)` 截断，从 `external_data_loader` 导入 `MAX_IMPORT_ROWS`。详见 `docs/dev-guides/13-unified-row-limits.md`。
...
- `size` 参数必须受 `MAX_IMPORT_ROWS`（200 万）硬上限约束，防止意外 OOM（参见 `docs/dev-guides/13-unified-row-limits.md`）
